...
var options = {
  ...
  method: 'POST', /*method*/
  headers: {
    'your-api-key': 'write*api*key*here*', /*write your api key*/
...
SensorTag.discover(function(sensorTag) {
	...
	function formatDate(d) {
        return d.getDate() + '-' + (show_now()) + "-" + d.getFullYear() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + " UTC";
	}
    ...
    function loop(){
        ...
        async.series([
            ...
            function(callback) {
                if (USE_READ) {
                    sensorTag.readIrTemperature(function(error, objectTemperature, ambientTemperature) {
                        ...
                        ambtemp = ambientTemperature.toFixed(1);

                        /*Forming JSON for temperature and POST to RESTful service*/
                        postdata = "write JSON, include sensor data and time with specified format of your RESTful web service";
                        var req = http.request(options, post_callback);
                        req.write(postdata);
                        req.end();
            ...
            function(callback) {
                if (USE_READ) {
                    sensorTag.readHumidity(function(error, temperature, humidity) {
                        ...
                        humd_temp = temperature.toFixed(1);
            ...
            function(callback) {
                var jsonTem = "[ { \"name\" : \"ObjTemp\", \"value\" :"+objtemp+" }, { \"name\" : \"AmbTemp\", \"value\" :"+ambtemp+" } ]";
                var jsonAcc = "[ { \"name : \"AccelX\", \"value\" :"+accelx+" }, { \"name\" : \"AccelY\", \"value\" :"+accely+" }, { \"name\" : \"AccelZ\", \"value\" :"+accelz+" } ]";
                var jsonHum = "[ { \"name\" : \"Humidity\", \"value\" :"+humd+" }, { \"name\" : \"Humidity_temp\", \"value\" :"+humd_temp+" } ]";
                var jsonMag = "[ { \"name\" : MagX, value :"+magx+" }, { name : MagY, value :"+magy+" }, { name : MagZ, value :"+magz+" } ]";
                var jsonBar = "[ { \"name\" : \"Barometric Pressure\", \"value\" :"+barp+" } ]";
                var jsonGyr = "[ { \"name\" : \"GyrX\", \"value\" :"+gyrx+" }, { \"name\" : \"GyrY\", \"value\" :"+gyry+" }, { \"name\" : \"GyrZ\", \"value\" :"+gyrz+" } ]";
                var jsonTime = "{\"time\" : \""+formatDate(new Date())+"\"}";
